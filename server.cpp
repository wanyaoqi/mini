...

{
	...
	bool b_tag=mySql.add_registered(name.c_str(),passwd.c_str(),id);//¨¨?ID
...

{
	...
	char **friend_list=mySql.get_friend_list(id,i);
...

{
	...
	char **file_list=mySql.get_file_list(id,i);
...

{
	...
	char **offmsg_list=mySql.get_offmsg(id,i);
...

{
	...
	bool logintag=mySql.user_login(id.c_str(),passwd.c_str());
...

{
	...
	bool md5_tag=mySql.judge_md5(md5.c_str());
...

{
...

	{
		...
		mySql.upload(m_user.id.c_str(),m_user.m_file.m_filename.c_str(),size,m_user.m_file.m_md5.c_str());//??¡Á??¨¦¨°?2?¨°a
...

	{
		...
		mySql.add_resume(m_user.id.c_str(),m_user.m_file.m_filename.c_str(),size-filesize);//?¨®??¦Ì?/////////
...

{
	...
	mySql.add_friend(m_user.id.c_str(),friendid.c_str());//?¡À?¨®?¨®¨¦?¨¢?
...

{
	...
	char *sntime=mySql.add_snopshot(m_user.id.c_str());///////
